...
DelugeConnection.prototype.addTorrent = function(url, cookie_domain, plugins, options) {
  ...
  // Chain promises for better error handling and response
  return this._connect()
    .then(() => {
...
DelugeConnection.prototype._addTorrentUrlToServer = function(url, options, cookie_domain) {
    ...
    return new Promise((resolve, reject) => {
        chrome.storage.local.get('send_cookies', data => {
            ...
            // If cookies are enabled, try direct URL method first
            if (data.send_cookies !== false) {
                console.log('[_addTorrentUrlToServer] Cookies enabled, trying direct URL method first');
                ...
                // If cookies are disabled, try direct URL method only
                console.log('[_addTorrentUrlToServer] Cookies disabled, using direct URL method only');
                this._addTorrentViaUrl(encodedUrl, params)
                    .then(resolve)
                    .catch(reject);
            }
